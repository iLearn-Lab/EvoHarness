...
EvoHarness 把 harness 暴露成一个可见、可扩展、可研究的工程表面，而不是隐藏在后台的运行时外壳。

核心表面一眼看：

- 🛠️ **26 tools**：files、shell、search、tasks、registry、MCP、subagents
- 🧾 **32 commands**：重复工作流的直接入口
- 🧠 **34 skills**：按需加载的过程性指导
- 🤖 **32 agents**：有边界的 delegation 与 inspection
- 🔌 **7 plugins**：workspace-native 的生态扩展
- 🛰️ **10 MCP servers / 29 MCP tools**：外部 tools、resources 与 prompts

在架构上，运行时把这些部分连成一体：

- terminal-native control 与 slash-command workflow
- `.claude/` 里的 markdown workflow surfaces
- `plugins/` 里的 plugin-native bundles
- MCP-native tools、resources 与 prompts
- long-horizon memory、analytics、approvals 与 evolution planning

这个项目的设计点很明确：**harness 本身就是主要研究对象。**
